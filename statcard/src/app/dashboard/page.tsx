...
type Profile = { firstName: string; lastName: string; username: string; accountType: AccountType; height: string; weight: string; graduatingClass: string; highSchool: string; gpa: string; sport: string; position: string; bio: string; hudl_highlight_url: string; hudl_secondary_urls: string[]; phone_number: string; contact_email: string; instagram_url: string; tiktok_url: string; youtube_url: string; x_url: string; stats: Stat[]; measurables: Stat[] };
const emptyProfile: Profile = { firstName: '', lastName: '', username: '', accountType: 'athlete', height: '', weight: '', graduatingClass: '', highSchool: '', gpa: '', sport: '', position: '', bio: '', hudl_highlight_url: '', hudl_secondary_urls: [], phone_number: '', contact_email: '', instagram_url: '', tiktok_url: '', youtube_url: '', x_url: '', stats: [], measurables: [] };
...
const asHudlUrls = (value: unknown): string[] => Array.isArray(value) ? value.filter((url): url is string => typeof url === 'string').slice(0, 5) : [];
...
export default function DashboardPage() {
  ...
  const hudlSecondaryErrors = profile.hudl_secondary_urls.map((url) => url.trim() && !isValidHudlUrl(url) ? 'Enter a valid Hudl video link.' : null);
  const hasHudlErrors = Boolean(hudlUrlError || hudlSecondaryErrors.some(Boolean));
  const readiness = useMemo(() => {
    const checks = [
      { label: 'Profile photo', detail: 'Use a clear, current athlete photo.', complete: Boolean(avatarUrl), weight: 10 },
      { label: 'Name and identity', detail: 'Add your first and last name.', complete: Boolean(profile.firstName.trim() && profile.lastName.trim()), weight: 10 },
      { label: 'Sport and position', detail: 'Help recruiters understand your fit quickly.', complete: Boolean(profile.sport.trim() && profile.position.trim()), weight: 15 },
      { label: 'Athlete story', detail: 'Write a short bio about your goals and strengths.', complete: Boolean(profile.bio.trim()), weight: 10 },
      { label: 'Highlight film', detail: 'Add a public Hudl link that shows your game.', complete: Boolean(profile.hudl_highlight_url.trim()) && !hudlUrlError, weight: 20 },
      { label: 'Performance stats', detail: 'Share at least one labeled stat with a value.', complete: profile.stats.some(({ label, value }) => label.trim() && value.trim()), weight: 15 },
      { label: 'Measurables', detail: 'Add at least one physical measurement.', complete: profile.measurables.some(({ label, value }) => label.trim() && value.trim()), weight: 10 },
      { label: 'School and class', detail: 'Add your high school and graduating class.', complete: Boolean(profile.highSchool.trim() && profile.graduatingClass.trim()), weight: 5 },
      { label: 'Contact details', detail: 'Give coaches a direct way to reach you.', complete: Boolean(profile.contact_email.trim() || profile.phone_number.trim()), weight: 5 },
    ];
    const points = checks.reduce((total, check) => total + (check.complete ? check.weight : 0), 0);
    return { checks, points, nextSteps: checks.filter((check) => !check.complete).slice(0, 3), recruiterReady: points >= 80 && checks[2].complete && checks[4].complete };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      let data: { first_name?: string | null; last_name?: string | null; username?: string | null; height?: string | null; weight?: string | null; graduating_class?: string | null; high_school?: string | null; gpa?: string | null; sport?: string | null; position?: string | null; bio?: string | null; avatar_url?: string | null; hudl_highlight_url?: string | null; hudl_secondary_urls?: unknown; phone_number?: string | null; contact_email?: string | null; instagram_url?: string | null; tiktok_url?: string | null; youtube_url?: string | null; x_url?: string | null; stats?: unknown; measurables?: unknown } | null = null;
  ...
  return <main className="min-h-screen pb-20 pt-8 sm:pt-10"><div className="page-shell max-w-5xl">
    ...
    <form onSubmit={saveProfile} className="space-y-6">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-bold text-slate-950">More highlight film</h2><p className="mt-1 text-sm text-slate-500">Add up to five secondary Hudl links. Your primary link always appears first on your public profile.</p></div><span className="text-xs font-bold text-slate-400">{profile.hudl_secondary_urls.length}/5</span></div><fieldset disabled={!isEditing} className="mt-5 space-y-3 disabled:opacity-70">{profile.hudl_secondary_urls.map((url, index) => <div key={index}><div className="flex gap-2"><input type="url" value={url} onChange={(event) => update('hudl_secondary_urls', profile.hudl_secondary_urls.map((current, currentIndex) => currentIndex === index ? event.target.value : current))} placeholder="https://www.hudl.com/video/..." className={`input min-w-0 flex-1 ${hudlSecondaryErrors[index] ? 'border-rose-400 focus:border-rose-500 focus:ring-rose-100' : ''}`} aria-label={`Secondary Hudl link ${index + 1}`} />{isEditing && <button type="button" onClick={() => update('hudl_secondary_urls', profile.hudl_secondary_urls.filter((_, currentIndex) => currentIndex !== index))} className="rounded-xl px-3 text-sm font-semibold text-rose-600 hover:bg-rose-50">Remove</button>}</div>{hudlSecondaryErrors[index] && <p className="mt-1 text-xs text-rose-600">{hudlSecondaryErrors[index]}</p>}</div>)}</fieldset>{isEditing && profile.hudl_secondary_urls.length < 5 && <button type="button" onClick={() => update('hudl_secondary_urls', [...profile.hudl_secondary_urls, ''])} className="mt-4 rounded-xl bg-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-800 hover:bg-slate-200">+ Add secondary link</button>}</section>
