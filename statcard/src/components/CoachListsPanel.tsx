...
async function fetchListSnapshot(): Promise<ListSnapshot> {
  const [{ data: listData, error: listError }, { data: memberData, error: memberError }] =
    await Promise.all([
      supabase
        .from('coach_lists')
  ...
  if (listError || memberError)
    return {
      lists: [],
      members: [],
      athletes: [],
      error: (listError ?? memberError)?.message ?? 'Unable to load lists.',
    };
...
export default function CoachListsPanel() {
  ...
  const createList = async () => {
    ...
    if (createError) setError(createError.message);
      ...
      setName('');
      applySnapshot(await fetchListSnapshot());
  ...
  const deleteList = async (list: List) => {
    ...
    if (deleteError) setError(deleteError.message);
      ...
      applySnapshot(await fetchListSnapshot());
      trackEvent('recruiting_list_action', { action: 'list_deleted' });
    }
  };
  ...
  return (
    <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm font-medium text-emerald-600">Recruiting workspace</p>
          <h2 className="mt-1 text-2xl font-bold tracking-tight text-slate-950">
            Your athlete lists
          </h2>
          <p className="mt-2 text-sm leading-6 text-slate-500">
            Organize athletes from their profiles into private lists tied to your coach account.
          </p>
        </div>
      ...
      {loading ? (
        <p className="mt-6 text-sm text-slate-500">Loading your lists…</p>
      ) : lists.length === 0 ? (
        <div className="mt-6 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-5 py-8 text-center text-sm text-slate-500">
          Create your first list, then add athletes from the directory.
        </div>
      ) : (
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {lists.map((list) => {
            ...
            return (
              <div key={list.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="font-bold text-slate-950">{list.name}</h3>
                    <p className="mt-1 text-xs text-slate-500">
                      {listMembers.length} {listMembers.length === 1 ? 'athlete' : 'athletes'}
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                {listMembers.length ? (
                  <ul className="mt-4 space-y-2">
                    {listMembers.map((athlete) => (
                      <li key={athlete.id}>
                        <Link
                          href={`/${athlete.username}`}
                          className="flex items-center justify-between rounded-xl bg-white px-3 py-2.5 text-sm font-semibold text-slate-700 hover:text-blue-700"
                        >
                          <span>
                  ...
                  <p className="mt-4 text-sm text-slate-500">No athletes added yet.</p>
                )}
              </div>
            );
          })}
