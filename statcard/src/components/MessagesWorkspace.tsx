...
async function loadInbox(): Promise<InboxSnapshot> {
  ...
  if (athleteResult.error || coachResult.error) {
    return {
      items: [],
      error:
        athleteResult.error?.message ?? coachResult.error?.message ?? 'Unable to load profiles.',
    };
  }
...
export default function MessagesWorkspace({
  ...
  useEffect(() => {
    ...
    const loadConversation = async () => {
      ...
      const { data, error } = await supabase
        ...
        .or(
          `and(sender_id.eq.${userId},recipient_id.eq.${selectedRecipientId}),and(sender_id.eq.${selectedRecipientId},recipient_id.eq.${userId})`,
        )
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const message = payload.new as DirectMessage;
          if (message.recipient_id === selectedRecipientId)
            setMessages((current) => addOrReplaceMessage(current, message));
          void refreshInbox();
        },
      )
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(({ person }) =>
      `${displayName(person)} ${person.username ?? ''}`.toLocaleLowerCase().includes(query),
    );
  }, [items, search]);
  ...
  return (
    <main className="px-0 py-0 sm:px-4 sm:py-6 lg:px-6">
      <div className="mx-auto flex h-[calc(100dvh-72px)] min-h-[580px] max-w-6xl overflow-hidden border-slate-200/80 bg-white shadow-card sm:h-[calc(100dvh-120px)] sm:rounded-[2rem] sm:border">
        <aside
          className={`${selectedItem || invalidRecipient ? 'hidden md:flex' : 'flex'} w-full shrink-0 flex-col border-r border-slate-200 md:w-[340px] lg:w-[390px]`}
        ...
        >
          <div className="border-b border-slate-200 px-5 pb-4 pt-5 sm:px-6 sm:pt-6">
            <div className="flex items-start justify-between gap-3">
              ...
              <span
                title={connected ? 'Realtime connected' : 'Realtime connecting'}
                className={`mt-1 size-2.5 rounded-full ring-4 ${connected ? 'bg-emerald-500 ring-emerald-100' : 'animate-pulse bg-amber-400 ring-amber-100'}`}
              >
          ...
          {inboxError ? (
            <ErrorPanel message={inboxError} retry={() => void refreshInbox(true)} />
            ...
            <div className="flex-1 overflow-y-auto">
              {filteredItems.map((item) => (
                <InboxLink
                  ...
                  active={item.person.id === selectedRecipientId}
                />
              ))}
            </div>
          ) : items.length ? (
            <div className="grid flex-1 place-items-center px-6 text-center">
              <div>
                <p className="font-bold text-slate-800">No connections found</p>
            ...
            <div className="grid flex-1 place-items-center px-7 text-center">
              <div>
                <div
                  aria-hidden="true"
                  className="mx-auto grid size-14 place-items-center rounded-full bg-blue-50 text-blue-600"
                >
                  <MessageIcon />
                </div>
                <h2 className="mt-4 text-lg font-bold text-slate-950">No conversations yet</h2>
                <p className="mt-2 text-sm leading-6 text-slate-500">
                  Once a follow is mutual, that person will appear here and you can start messaging.
                </p>
                <Link
                  href="/friends"
        ...
        <section
          className={`${selectedItem || invalidRecipient ? 'flex' : 'hidden md:flex'} min-w-0 flex-1 flex-col bg-slate-50`}
          aria-label={
            selectedItem ? `Conversation with ${displayName(selectedItem.person)}` : 'Message area'
          }
        >
          {invalidRecipient ? (
            <InvalidConversation />
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto px-4 py-5 sm:px-6" aria-live="polite">
                {loadingConversation || loadedConversationId !== selectedRecipientId ? (
                  ...
                  <ErrorPanel
                    ...
                    retry={() => setConversationRefreshKey((current) => current + 1)}
                  />
                ) : messages.length ? (
                    ...
                    {messages.map((message, index) => (
                      <MessageBubble
                        ...
                        showReadReceipt={
                          ...
                          !messages.slice(index + 1).some((later) => later.sender_id === userId)
                        }
                      />
                    ))}
                  ...
                  <div className="grid h-full place-items-center text-center">
                    <div>
                      <div
                        aria-hidden="true"
                        className="mx-auto grid size-14 place-items-center rounded-full bg-white text-blue-600 shadow-sm"
                      >
                        <MessageIcon />
                      </div>
                      ...
                      <p className="mt-1 text-sm text-slate-500">
                        Send a text message to {displayName(selectedItem.person)}.
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div className="border-t border-slate-200 bg-white p-3 sm:p-4">
                {conversationError && messages.length > 0 && (
                  <p
                    role="alert"
                    className="mx-auto mb-2 max-w-3xl text-sm font-medium text-rose-600"
                  >
                    {conversationError}
                  </p>
                )}
                <div className="mx-auto flex max-w-3xl items-end gap-2">
                  <label className="min-w-0 flex-1">
                    <span className="sr-only">Message {displayName(selectedItem.person)}</span>
                    <textarea
                      value={draft}
                      ...
                      rows={1}
                      maxLength={2000}
                      placeholder={`Message ${displayName(selectedItem.person)}`}
                  ...
                  <button
                    ...
                    className="grid size-11 shrink-0 place-items-center rounded-xl bg-brand-600 text-white shadow-lg shadow-brand-600/20 transition hover:-translate-y-0.5 hover:bg-brand-700 disabled:cursor-not-allowed disabled:bg-slate-300 disabled:shadow-none"
                  >
                    <SendIcon />
                  </button>
                </div>
            ...
            <div className="grid flex-1 place-items-center px-8 text-center">
              <div>
                <div
                  aria-hidden="true"
                  className="mx-auto grid size-16 place-items-center rounded-2xl bg-white text-blue-600 shadow-sm"
                >
                  <MessageIcon />
                </div>
                <h2 className="mt-5 text-xl font-bold text-slate-950">Select a conversation</h2>
                <p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">
                  Choose one of your mutual connections to view your messages or start a
                  conversation.
                </p>
              </div>
            </div>
...
function InboxLink({
  ...
  return (
    <Link
      ...
      className={`flex gap-3 border-b border-slate-100 px-5 py-4 outline-none transition hover:bg-slate-50 focus-visible:ring-4 focus-visible:ring-inset focus-visible:ring-blue-100 sm:px-6 ${active ? 'bg-blue-50 hover:bg-blue-50' : ''}`}
    >
      <div className="relative shrink-0">
        ...
        <span
          ...
          className={`absolute bottom-0 right-0 size-3.5 rounded-full border-2 border-white ${isOnline ? 'bg-emerald-500' : 'bg-slate-300'}`}
        />
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-2">
          <p
            className={`truncate text-sm text-slate-950 ${item.unreadCount ? 'font-extrabold' : 'font-bold'}`}
          >
            {name}
          </p>
          {item.lastMessageAt && (
            <time
              ...
              className={`shrink-0 text-xs ${item.unreadCount ? 'font-bold text-blue-700' : 'text-slate-400'}`}
            >
        ...
        <div className="mt-1 flex items-center gap-2">
          <p
            className={`min-w-0 flex-1 truncate text-sm ${item.unreadCount ? 'font-semibold text-slate-800' : 'text-slate-500'}`}
          >
            {preview}
          </p>
          {item.unreadCount > 0 && (
            <span
              ...
              className="min-w-5 rounded-full bg-blue-600 px-1.5 py-0.5 text-center text-[11px] font-bold text-white"
            >
...
function ConversationHeader({ person }: { person: Person }) {
  ...
  return (
    <header className="flex h-[77px] shrink-0 items-center gap-3 border-b border-slate-200 bg-white px-4 sm:px-6">
      <Link
        href="/messages"
        className="grid size-9 shrink-0 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 md:hidden"
      ...
      <div className="relative">
        ...
        <span className="absolute bottom-0 right-0">
          <span className="sr-only">Online status is shown next to the name</span>
        </span>
      </div>
      ...
      {person.username && (
        <Link
          ...
          className="ml-auto shrink-0 rounded-lg px-3 py-2 text-xs font-bold text-blue-700 hover:bg-blue-50"
        >
          View profile
        </Link>
      )}
...
function MessageBubble({
  ...
  return (
    <div className={`flex ${own ? 'justify-end' : 'justify-start'}`}>
      <div className={`max-w-[85%] sm:max-w-[72%] ${own ? 'text-right' : 'text-left'}`}>
        <div
          className={`inline-block whitespace-pre-wrap break-words rounded-2xl px-4 py-2.5 text-left text-sm leading-6 shadow-sm ${own ? 'rounded-br-md bg-blue-600 text-white' : 'rounded-bl-md border border-slate-200 bg-white text-slate-800'}`}
        >
          {message.body}
        </div>
        <div
          className={`mt-1 flex items-center gap-1.5 px-1 text-[11px] text-slate-400 ${own ? 'justify-end' : 'justify-start'}`}
        >
          <time dateTime={message.created_at}>{formatMessageTime(message.created_at)}</time>
          {own && showReadReceipt && message.read_at && <span>· Read</span>}
        </div>
      </div>
    </div>
  );
...
function ErrorPanel({ message, retry }: { message: string; retry: () => void }) {
  return (
    <div
      role="alert"
      className="m-5 rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-sm text-rose-700"
    >
      <p>{message}</p>
      <button
        type="button"
        onClick={retry}
        className="mt-2 font-bold underline decoration-rose-300 underline-offset-4"
      >
        Try again
      </button>
    </div>
  ...
  return (
    <div className="grid flex-1 place-items-center px-6 text-center">
      <div>
        <div
          aria-hidden="true"
          className="mx-auto grid size-14 place-items-center rounded-full bg-amber-50 text-2xl text-amber-600"
        >
          !
        </div>
        <h1 className="mt-4 text-xl font-bold text-slate-950">Conversation unavailable</h1>
        <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
          You can only message people who are currently mutual connections.
        </p>
        <Link
          href="/messages"
          className="mt-5 inline-flex rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800"
