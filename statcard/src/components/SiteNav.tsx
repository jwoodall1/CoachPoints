...
export default function SiteNav() {
  ...
  return <nav className="border-b border-slate-200 bg-white/90 px-4 py-3 backdrop-blur sm:px-6" aria-label="Main navigation">
    <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-x-4 gap-y-2">
      <div className="flex min-w-0 items-center">
        <Link href="/" aria-label="Athlio home" className="inline-flex shrink-0 items-center"><Image src="/athlio-logo.png" alt="Athlio" width={132} height={33} className="h-7 w-auto object-contain sm:h-8" loading="eager" /></Link>
        {ready && userId && username && <span className={`ml-3 inline-flex items-center gap-1.5 border-l border-slate-200 pl-3 text-xs font-bold uppercase tracking-[0.14em] ${accountType === 'coach' ? 'text-emerald-700' : 'text-blue-700'}`}><span className={`size-2 rounded-full ${accountType === 'coach' ? 'bg-emerald-500' : 'bg-blue-500'}`} />{accountType}</span>}
      </div>
      <div className={`flex items-center justify-start gap-1 overflow-x-auto sm:w-auto sm:justify-end sm:gap-2 ${ready && userId && username ? 'w-full' : 'w-auto'}`}>
        <Link href="/" className="rounded-lg px-3 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-100 hover:text-slate-950">Home</Link>
