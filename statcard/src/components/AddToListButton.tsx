...
export default function AddToListButton({
  ...
  useEffect(() => {
    ...
    const positionPopover = () => {
      ...
      const edge = 16;
      const left = Math.min(
        Math.max(edge, anchorRect.right - popoverWidth),
        window.innerWidth - popoverWidth - edge,
      );
      const below = anchorRect.bottom + gap;
      const above = anchorRect.top - popover.offsetHeight - gap;
      const top =
        below + popover.offsetHeight <= window.innerHeight - edge || above < edge ? below : above;
  ...
  const loadLists = async () => {
    setLoading(true);
    setError(null);
    const [{ data: listsData, error: listsError }, { data: memberData, error: memberError }] =
      await Promise.all([
  ...
  const toggleMembership = async (list: CoachList) => {
    ...
    const result = isMember
      ...
      : await supabase
          .from('coach_list_members')
          .insert({ list_id: list.id, athlete_id: athleteId });
    if (result.error) setError(result.error.message);
    else {
      setMemberListIds((current) =>
        isMember ? current.filter((id) => id !== list.id) : [...current, list.id],
      );
      trackEvent('recruiting_list_action', {
  ...
  return (
    <div ref={pickerRef} className="relative" onClick={(event) => event.stopPropagation()}>
      <button
        type="button"
        onClick={openPicker}
      ...
      {open &&
        ...
        createPortal(
          ...
          >
            ...
            {loading ? (
              <p className="mt-4 text-sm text-slate-500">Loading lists…</p>
            ) : lists.length ? (
