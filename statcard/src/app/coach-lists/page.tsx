...
export default function CoachListsPage() {
  ...
  useEffect(() => {
    ...
    root.render(<PipelineEditor athlete={editingAthlete} value={pipelineByAthlete.get(editingAthlete.id)} lists={lists} members={members} currentListId={selectedListId} onClose={() => setEditingAthlete(null)} onSave={(values) => void savePipeline(editingAthlete.id, values)} onMove={(targetListId) => void moveAthlete(editingAthlete.id, targetListId)} />);
    return () => { root.unmount(); host.remove(); };
  }, [editingAthlete, lists, members, moveAthlete, pipelineByAthlete, savePipeline, selectedListId]);
  ...
  return <main className="min-h-screen pb-20 pt-8 sm:pt-10"><div className="page-shell max-w-[1600px]">
    <header className="athletic-grid relative mb-8 overflow-hidden rounded-[2rem] bg-slate-950 px-6 py-8 text-white shadow-2xl shadow-slate-300/50 sm:px-8 sm:py-10"><div className="absolute -right-16 -top-24 size-72 rounded-full bg-emerald-500/20 blur-3xl" /><div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><div><Link href="/coach-dashboard" className="inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-wider text-emerald-300 hover:text-emerald-200"><ArrowLeft className="size-3.5" />Coach dashboard</Link><div className="mt-5 flex items-start gap-4"><span className="hidden size-12 place-items-center rounded-2xl bg-emerald-400/15 text-emerald-200 sm:grid"><ListChecks className="size-5" /></span><div><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-emerald-300">Recruiting workspace</p><h1 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">Athlete lists</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">Build focused prospect groups, compare athlete details, and message qualified lists.</p></div></div></div><div className="flex w-full gap-2 lg:w-auto"><input value={newListName} onChange={(event) => setNewListName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') createList(); }} placeholder="Name a new list" className="input min-w-0 bg-white text-slate-900 lg:w-60" maxLength={80} /><button type="button" disabled={!newListName.trim() || saving} onClick={createList} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-emerald-500 px-4 py-2.5 text-sm font-extrabold text-white shadow-lg shadow-emerald-500/20 transition hover:bg-emerald-400 disabled:opacity-50"><Plus className="size-4" />Create</button></div></div></header>
...
function PipelineEditor({ athlete, value, lists, members, currentListId, onClose, onSave, onMove }: { athlete: Athlete; value?: Pipeline; lists: List[]; members: Member[]; currentListId: string; onClose: () => void; onSave: (values: Pick<Pipeline, 'stage' | 'notes' | 'tags' | 'reminder' | 'follow_up_date'>) => void; onMove: (targetListId: string) => void }) {
  ...
  const otherLists = lists.filter((list) => list.id !== currentListId && !members.some((member) => member.list_id === list.id && member.athlete_id === athlete.id));
  const name = [athlete.first_name, athlete.last_name].filter(Boolean).join(' ') || athlete.username;
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section role="dialog" aria-modal="true" aria-labelledby="pipeline-editor-title" className="max-h-[calc(100vh-2rem)] w-full max-w-xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-emerald-600">Recruiting pipeline</p><h2 id="pipeline-editor-title" className="mt-1 text-2xl font-black tracking-tight text-slate-950">{name}</h2><p className="mt-2 text-sm text-slate-500">These details follow the athlete across every list.</p></div><button type="button" onClick={onClose} aria-label="Close pipeline editor" className="grid size-9 shrink-0 place-items-center rounded-xl text-xl text-slate-400 hover:bg-slate-100 hover:text-slate-700">×</button></div><label className="mt-6 block"><span className="mb-2 block text-sm font-bold text-slate-700">Stage</span><select value={stage} onChange={(event) => setStage(event.target.value as Stage)} className="input">{(['New', 'Interested', 'Contacted', 'Evaluating', 'Offer', 'Archived'] as Stage[]).map((option) => <option key={option}>{option}</option>)}</select></label><label className="mt-4 block"><span className="mb-2 block text-sm font-bold text-slate-700">Private notes</span><textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={4} maxLength={4000} placeholder="Add context for your staff…" className="input resize-y" /></label><label className="mt-4 block"><span className="mb-2 block text-sm font-bold text-slate-700">Tags</span><input value={tags} onChange={(event) => setTags(event.target.value)} placeholder="needs film, academic fit, priority" className="input" /><span className="mt-1 block text-xs text-slate-400">Separate tags with commas.</span></label><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">Follow-up date</span><input type="date" value={followUp} onChange={(event) => setFollowUp(event.target.value)} className="input" /></label><label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">Reminder</span><input value={reminder} onChange={(event) => setReminder(event.target.value)} placeholder="Call, email, review film…" className="input" /></label></div><div className="mt-6 rounded-2xl border border-blue-100 bg-blue-50/60 p-4"><p className="text-sm font-bold text-slate-800">Move to another list</p><div className="mt-2 flex flex-col gap-2 sm:flex-row"><select value={targetListId} onChange={(event) => setTargetListId(event.target.value)} className="input"><option value="">Keep in this list</option>{otherLists.map((list) => <option key={list.id} value={list.id}>{list.name}</option>)}</select>{targetListId && <button type="button" onClick={() => { onMove(targetListId); onClose(); }} className="btn-secondary whitespace-nowrap">Move athlete</button>}</div>{!otherLists.length && <p className="mt-2 text-xs text-slate-500">This athlete is already in every other list.</p>}</div><div className="mt-7 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={onClose} className="btn-secondary">Cancel</button><button type="button" onClick={() => onSave({ stage, notes, tags: tags.split(',').map((tag) => tag.trim()).filter(Boolean), reminder: reminder || null, follow_up_date: followUp || null })} className="btn-primary">Save pipeline</button></div></section></div>;
}
