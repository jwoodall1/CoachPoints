...
export default async function AthleteProfile({ params }: { params: Promise<{ username: string }> }) {
  ...
  const { data: athleteProfile, error: athleteError } = await supabase
    .from('profiles')
    .select('first_name, last_name, account_type, sport, bio, avatar_url, hudl_highlight_url, instagram_url, tiktok_url, youtube_url, x_url, stats, measurables')
  ...
  if (!profile) {
    const { data: coachProfile, error: coachError } = await supabase
      ...
      .eq('username', username)
      .maybeSingle();
    profile = coachProfile ? { ...coachProfile, account_type: 'coach', hudl_highlight_url: null, stats: null, measurables: null } : null;
    error = coachError;
  }
