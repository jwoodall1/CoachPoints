...
export default function HomePage() {
  ...
  const filteredAthletes = useMemo(() => {
    ...
    return athletes.filter((athlete) => {
      ...
      return (!query || name.includes(query) || athlete.username.toLowerCase().includes(query)) && (sportFilter === 'all' || athlete.sport === sportFilter) && (positionFilter === 'all' || athlete.position === positionFilter);
    });
  }, [athletes, search, sportFilter, positionFilter]);
  ...
  return <main className="min-h-[calc(100vh-125px)] bg-slate-50 px-4 py-10 sm:px-6 lg:py-14"><div className="mx-auto max-w-5xl">
    ...
    {error ? <div className="rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-sm text-rose-700">{error}</div> : athletes.length === 0 ? <EmptyState /> : <>
      <label className="mb-6 block max-w-xs"><span className="mb-2 block text-sm font-semibold text-slate-700">Search by position</span><select value={positionFilter} onChange={(event) => setPositionFilter(event.target.value)} className="input"><option value="all">All positions</option>{positions.map((position) => <option key={position} value={position}>{position}</option>)}</select></label>
