...
type CoachProfile = { firstName: string; lastName: string; username: string; sport: string; bio: string; instagramUrl: string; tiktokUrl: string; youtubeUrl: string; xUrl: string };
const emptyProfile: CoachProfile = { firstName: '', lastName: '', username: '', sport: '', bio: '', instagramUrl: '', tiktokUrl: '', youtubeUrl: '', xUrl: '' };
...
export default function CoachDashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data } = await supabase.from('coachprofiles').select('first_name, last_name, username, sport, bio, instagram_url, tiktok_url, youtube_url, x_url').eq('id', session.user.id).maybeSingle();
      setProfile({ firstName: data?.first_name ?? session.user.user_metadata.first_name ?? '', lastName: data?.last_name ?? session.user.user_metadata.last_name ?? '', username: data?.username ?? session.user.user_metadata.username ?? '', sport: data?.sport ?? '', bio: data?.bio ?? '', instagramUrl: data?.instagram_url ?? '', tiktokUrl: data?.tiktok_url ?? '', youtubeUrl: data?.youtube_url ?? '', xUrl: data?.x_url ?? '' });
      setLoading(false);
    };
    load();
  }, [router]);
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-8 sm:px-6 lg:py-12"><div className="mx-auto max-w-5xl">
    ...
    <form onSubmit={save} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="text-xl font-bold text-slate-950">Coach information</h2><p className="mt-1 text-sm text-slate-500">Share the information athletes need to know about you.</p></div>{editing && <button type="button" onClick={() => setEditing(false)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Cancel</button>}</div>
      <div className="grid gap-5 sm:grid-cols-2"><Field label="First name" value={profile.firstName} onChange={(value) => update('firstName', value)} placeholder="Jordan" disabled={!editing} required /><Field label="Last name" value={profile.lastName} onChange={(value) => update('lastName', value)} placeholder="Lee" disabled={!editing} required /><Field label="Sport or program" value={profile.sport} onChange={(value) => update('sport', value)} placeholder="Basketball" disabled={!editing} /><label className="sm:col-span-2 block text-sm font-semibold text-slate-700"><span className="mb-2 block">Bio</span><textarea value={profile.bio} onChange={(event) => update('bio', event.target.value)} rows={5} className="input resize-y disabled:cursor-not-allowed disabled:bg-slate-100" placeholder="Your coaching experience, philosophy, and specialties." disabled={!editing} /></label><Field label="Instagram URL" value={profile.instagramUrl} onChange={(value) => update('instagramUrl', value)} placeholder="https://instagram.com/yourname" disabled={!editing} /><Field label="TikTok URL" value={profile.tiktokUrl} onChange={(value) => update('tiktokUrl', value)} placeholder="https://tiktok.com/@yourname" disabled={!editing} /><Field label="YouTube URL" value={profile.youtubeUrl} onChange={(value) => update('youtubeUrl', value)} placeholder="https://youtube.com/@yourname" disabled={!editing} /><Field label="X URL" value={profile.xUrl} onChange={(value) => update('xUrl', value)} placeholder="https://x.com/yourname" disabled={!editing} /></div>
      <div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">{notice ? <p role="status" className={`text-sm font-medium ${notice.includes('successfully') ? 'text-emerald-600' : 'text-rose-600'}`}>{notice}</p> : <span />}{editing ? <button disabled={saving} className="rounded-xl bg-emerald-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 disabled:opacity-60">{saving ? 'Saving…' : 'Save coach profile'}</button> : <button type="button" onClick={() => setEditing(true)} className="rounded-xl bg-slate-950 px-6 py-3 text-sm font-semibold text-white hover:bg-slate-800">Edit profile</button>}</div>
    </form>
  </div></main>;
}
