...
const emptyInstitution = (): Institution => ({
  ...
  primary_color: '#0f172a', secondary_color: '#e2e8f0', competition_level: null, tagline: '', about: '',
...
export default function InstitutionAdminPage() {
  ...
  const save = async () => {
    ...
    try {
      const payload = {
        ...
        competition_level: selected.competition_level || null,
  ...
  const deleteInstitution = async () => {
    if (!selected?.id) return;
    const name = textValue(selected.name).trim() || 'this institution';
    if (!window.confirm(`Delete ${name}? This will also remove its sports, admins, and requests.`)) return;
    setSaving(true); setNotice(null);
    const { error } = await withTimeout(supabase.from('institutions').delete().eq('id', selected.id), 'Deleting institution');
    setSaving(false);
    if (error) { setNotice(error.message); return; }
    setInstitutions((current) => current.filter((institution) => institution.id !== selected.id));
    setSelected(null); setAdmins([]); setNotice(`${name} deleted successfully.`);
  };
    ...
    <main className="min-h-screen pb-20 pt-8 sm:pt-10">
      <div className="page-shell max-w-7xl">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[18rem_1fr]">
          <aside className="surface-card p-3"><p className="px-3 py-2 text-xs font-extrabold uppercase tracking-wider text-slate-500">Institutions ({institutions.length})</p>{institutions.map((item) => <button key={item.id} type="button" onClick={() => selectInstitution(item)} className={`flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition ${selected?.id === item.id ? 'bg-slate-950 text-white' : 'hover:bg-slate-50'}`}><span className="grid size-10 shrink-0 place-items-center rounded-xl" style={{ backgroundColor: item.logo_url ? 'transparent' : item.primary_color }}>{item.logo_url ? <img src={item.logo_url} alt="" className="size-8 object-contain" /> : <Building2 className="size-4 text-white" />}</span><span className="min-w-0"><span className="block truncate text-sm font-black">{item.name}</span><span className={`block text-xs ${selected?.id === item.id ? 'text-slate-300' : 'text-slate-500'}`}>{item.status}</span></span></button>)}</aside>
          {selected ? <Editor institution={selected} update={update} save={save} deleteInstitution={deleteInstitution} saving={saving} addSport={() => update('sports', [...selected.sports, emptySport()])} removeSport={(index) => update('sports', selected.sports.filter((_, current) => current !== index))} onUpload={() => setLogoModalOpen(true)} admins={admins} accounts={accounts} selectedAdminId={selectedAdminId} setSelectedAdminId={setSelectedAdminId} addAdmin={addAdmin} removeAdmin={removeAdmin} /> : <div className="surface-card grid min-h-96 place-items-center p-8 text-center"><div><Building2 className="mx-auto size-10 text-slate-300" /><p className="mt-4 font-bold text-slate-600">Select an institution or create a new one.</p></div></div>}
        </div>
      </div>
...
function Editor({ institution, update, save, deleteInstitution, saving, addSport, removeSport, onUpload, admins, accounts, selectedAdminId, setSelectedAdminId, addAdmin, removeAdmin }: EditorProps) {
  return <section className="surface-card p-6 sm:p-8"><div className="flex flex-col gap-4 border-b border-slate-100 pb-6 sm:flex-row sm:items-start sm:justify-between"><div><p className="eyebrow">Institution details</p><h2 className="mt-2 text-2xl font-black text-slate-950">{institution.name || 'New institution'}</h2></div><div className="flex flex-wrap gap-2"><Link href={institution.slug ? `/institutions/${institution.slug}` : '#'} className={`btn-secondary ${institution.slug ? '' : 'pointer-events-none opacity-50'}`}><Eye className="size-4" /> Preview</Link>{institution.id && <button type="button" onClick={deleteInstitution} disabled={saving} className="btn-secondary border-rose-200 text-rose-700 hover:bg-rose-50"><Trash2 className="size-4" /> Delete</button>}<button type="button" onClick={save} disabled={saving} className="btn-primary"><Save className="size-4" /> {saving ? 'Saving…' : 'Save institution'}</button></div></div>
    ...
    <label className="mt-6 block max-w-sm"><span className="mb-2 block text-xs font-extrabold uppercase tracking-wider text-slate-500">Competition level</span><select className="input" value={institution.competition_level ?? ''} onChange={(event) => update('competition_level', event.target.value || null)}><option value="">Not specified</option>{competitionLevels.map((level) => <option key={level} value={level}>{level}</option>)}</select></label><TextArea label="Tagline" value={institution.tagline} onChange={(v: string) => update('tagline', v)} /><TextArea label="About" value={institution.about} onChange={(v: string) => update('about', v)} /><TextArea label="Admissions requirements" value={institution.admissions_requirements} onChange={(v: string) => update('admissions_requirements', v)} />
