  ...
  return <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/85 backdrop-blur-xl supports-[backdrop-filter]:bg-white/75">
    <nav className="page-shell" aria-label="Main navigation" onClick={(event) => { if ((event.target as HTMLElement).closest('a')) setMobileOpen(false); }}>
      <div className="flex h-18 items-center justify-between gap-5">
        <div className="flex min-w-0 items-center gap-4"><Link href="/" aria-label="CoachPoints home" className="group inline-flex shrink-0 items-center rounded-lg"><Image src="/coachpoints-logo.png" alt="CoachPoints" width={190} height={65} className="h-10 w-auto object-contain transition group-hover:opacity-80" priority /></Link>{signedIn && <span className={`hidden items-center gap-2 rounded-full border px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.16em] sm:inline-flex ${accountType === 'coach' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-brand-200 bg-brand-50 text-brand-700'}`}><span className={`size-1.5 rounded-full ${accountType === 'coach' ? 'bg-emerald-500' : 'bg-brand-500'}`} />{accountType}</span>}</div>
