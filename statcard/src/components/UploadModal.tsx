...
export default function UploadModal({ isOpen, onClose, onSave }: UploadModalProps) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Edit profile photo">
    <div className="w-full max-w-lg rounded-[2rem] bg-white p-6 shadow-2xl sm:p-8"><div className="mb-6 flex items-start justify-between"><div><p className="eyebrow">Profile editor</p><h2 className="mt-2 text-xl font-black text-slate-950">Profile photo</h2><p className="mt-1 text-sm text-slate-500">Position and zoom your image inside the circle.</p></div><button type="button" onClick={onClose} className="grid size-9 place-items-center rounded-xl text-slate-400 hover:bg-slate-100 hover:text-slate-700" aria-label="Close"><X className="size-5" /></button></div>
      {!source ? <label className="grid cursor-pointer place-items-center rounded-3xl border-2 border-dashed border-slate-300 bg-slate-50 px-6 py-14 text-center transition hover:border-brand-400 hover:bg-brand-50"><span className="grid size-12 place-items-center rounded-2xl bg-white text-brand-700 shadow-sm"><ImagePlus className="size-5" /></span><span className="mt-4 text-sm font-extrabold text-slate-800">Choose a photo</span><span className="mt-2 text-xs text-slate-500">JPG, PNG, or WebP</span><input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={chooseFile} /></label> : <><div onPointerDown={beginDrag} onPointerMove={moveDrag} onPointerUp={() => { dragStart.current = null; }} className="relative mx-auto size-70 touch-none cursor-grab overflow-hidden rounded-full bg-slate-200 shadow-inner ring-8 ring-slate-100 active:cursor-grabbing"><img ref={imageRef} src={source} alt="Crop preview" draggable={false} onLoad={(event) => setNaturalSize({ width: event.currentTarget.naturalWidth, height: event.currentTarget.naturalHeight })} style={{ width: imageWidth, height: imageHeight, transform: `translate3d(${imageX}px, ${imageY}px, 0)` }} className="pointer-events-none absolute left-0 top-0 max-w-none select-none will-change-transform" /></div><label className="mx-auto mt-6 block max-w-70 text-sm font-bold text-slate-700">Zoom<input type="range" min="1" max="3" step="0.01" value={zoom} onChange={(event) => { const nextZoom = Number(event.target.value); setZoom(nextZoom); setOffset((current) => clampOffset(current, nextZoom)); }} className="mt-3 w-full accent-brand-600" /></label><label className="mt-4 block text-center text-sm font-extrabold text-brand-600 hover:text-brand-700"><span>Choose a different photo</span><input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={chooseFile} /></label></>}
