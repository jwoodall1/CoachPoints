...
function MarketingHome() {
  ...
  return <main className="overflow-hidden bg-white">
    ...
    <section className="page-shell py-20"><div className="relative overflow-hidden rounded-[2rem] bg-brand-600 px-6 py-12 text-center text-white shadow-2xl shadow-brand-600/20 sm:px-12"><div className="athletic-grid absolute inset-0" /><div className="relative"><p className="text-xs font-extrabold uppercase tracking-[0.2em] text-brand-100">Ready when you are</p><h2 className="mx-auto mt-3 max-w-2xl text-3xl font-black tracking-tight sm:text-4xl">Put your next opportunity in motion.</h2><p className="mx-auto mt-4 max-w-xl text-brand-100">Create your CoachPoints profile and start building the network around your goals.</p><Link href="/login?role=athlete" className="mt-7 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-white px-6 py-3 text-sm font-extrabold text-brand-700 shadow-xl transition hover:-translate-y-0.5 hover:bg-brand-50">Build your profile <ArrowRight className="size-4" /></Link></div></div></section>
  </main>;
}
...
function EmptyState() { return <div className="surface-card px-6 py-16 text-center"><UsersRound className="mx-auto size-10 text-brand-600" /><h2 className="mt-4 text-xl font-black text-slate-950">The directory is ready for its first profile</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">Complete your public profile to start building the CoachPoints network.</p><Link href="/dashboard" className="btn-primary mt-6">Complete profile <ArrowRight className="size-4" /></Link></div>; }
