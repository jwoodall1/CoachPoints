...
export default function CoachDashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data } = await supabase.from('coachprofiles').select('first_name, last_name, username, college_university, sport, bio, instagram_url, tiktok_url, youtube_url, x_url').eq('id', session.user.id).maybeSingle();
      setProfile({ firstName: data?.first_name ?? session.user.user_metadata.first_name ?? '', lastName: data?.last_name ?? session.user.user_metadata.last_name ?? '', username: data?.username ?? session.user.user_metadata.username ?? '', collegeUniversity: data?.college_university ?? '', sport: data?.sport ?? '', bio: data?.bio ?? '', instagramUrl: data?.instagram_url ?? '', tiktokUrl: data?.tiktok_url ?? '', youtubeUrl: data?.youtube_url ?? '', xUrl: data?.x_url ?? '' });
      setLoading(false);
    };
    load();
  }, [router]);
