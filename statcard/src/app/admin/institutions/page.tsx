...
export default function InstitutionAdminPage() {
  ...
  const addAdmin = async () => {
    if (!selected?.id || !adminEmail.trim()) return;
    const { error } = await supabase.rpc('assign_institution_admin', { p_institution_id: selected.id, p_email: adminEmail.trim() });
    if (error) return setNotice(error.message);
    setAdminEmail(''); await selectInstitution(selected); setNotice('Institution admin assigned.');
  };
    ...
    <main className="min-h-screen pb-20 pt-8 sm:pt-10">
      <div className="page-shell max-w-7xl">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[18rem_1fr]">
          ...
          {selected ? <Editor institution={selected} update={update} save={save} saving={saving} addSport={() => update('sports', [...selected.sports, emptySport()])} removeSport={(index) => update('sports', selected.sports.filter((_, current) => current !== index))} onUpload={() => setLogoModalOpen(true)} admins={admins} adminEmail={adminEmail} setAdminEmail={setAdminEmail} addAdmin={addAdmin} removeAdmin={removeAdmin} /> : <div className="surface-card grid min-h-96 place-items-center p-8 text-center"><div><Building2 className="mx-auto size-10 text-slate-300" /><p className="mt-4 font-bold text-slate-600">Select an institution or create a new one.</p></div></div>}
        </div>
      </div>
...
function Editor({ institution, update, save, saving, addSport, removeSport, onUpload, admins, adminEmail, setAdminEmail, addAdmin, removeAdmin }: EditorProps) {
