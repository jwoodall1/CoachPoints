...
type Profile = { firstName: string; lastName: string; username: string; accountType: AccountType; height: string; weight: string; graduatingClass: string; gpa: string; sport: string; position: string; bio: string; hudl_highlight_url: string; phone_number: string; contact_email: string; instagram_url: string; tiktok_url: string; youtube_url: string; x_url: string; stats: Stat[]; measurables: Stat[] };
const emptyProfile: Profile = { firstName: '', lastName: '', username: '', accountType: 'athlete', height: '', weight: '', graduatingClass: '', gpa: '', sport: '', position: '', bio: '', hudl_highlight_url: '', phone_number: '', contact_email: '', instagram_url: '', tiktok_url: '', youtube_url: '', x_url: '', stats: [], measurables: [] };
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      let data: { first_name?: string | null; last_name?: string | null; username?: string | null; height?: string | null; weight?: string | null; graduating_class?: string | null; gpa?: string | null; sport?: string | null; position?: string | null; bio?: string | null; avatar_url?: string | null; hudl_highlight_url?: string | null; phone_number?: string | null; contact_email?: string | null; instagram_url?: string | null; tiktok_url?: string | null; youtube_url?: string | null; x_url?: string | null; stats?: unknown; measurables?: unknown } | null = null;
  ...
  const saveProfile = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const error = profile.accountType === 'coach'
      ...
      : (await supabase.from('profiles').upsert({ id: user.id, first_name: profile.firstName.trim(), last_name: profile.lastName.trim(), username, height: profile.height.trim() || null, weight: profile.weight.trim() || null, graduating_class: profile.graduatingClass.trim() || null, gpa: profile.gpa.trim() || null, sport: profile.sport.trim(), position: getPositions(profile.sport).includes(profile.position) ? profile.position : null, bio: profile.bio.trim(), hudl_highlight_url: profile.hudl_highlight_url.trim() || null, phone_number: profile.phone_number.trim() || null, contact_email: profile.contact_email.trim() || null, instagram_url: profile.instagram_url.trim() || null, tiktok_url: profile.tiktok_url.trim() || null, youtube_url: profile.youtube_url.trim() || null, x_url: profile.x_url.trim() || null, stats, measurables: Object.fromEntries(profile.measurables.filter(({ label }) => label.trim()).map(({ label, value }) => [label.trim(), value.trim()])) }, { onConflict: 'id' })).error;
