...
import { ArrowUpRight, CheckCircle2, Circle, Dumbbell, ListChecks, LogOut, Pencil, Save, Sparkles, UsersRound } from 'lucide-react';
...
export default function CoachDashboardPage() {
  ...
  const readiness = (() => {
    const checks = [
      { label: 'Name', detail: 'Add your first and last name.', complete: Boolean(profile.firstName.trim() && profile.lastName.trim()), weight: 20 },
      { label: 'School or program', detail: 'Tell athletes which program you represent.', complete: Boolean(profile.collegeUniversity.trim()), weight: 20 },
      { label: 'Sport or program', detail: 'Make your recruiting focus clear.', complete: Boolean(profile.sport.trim()), weight: 15 },
      { label: 'Coaching story', detail: 'Share your experience and philosophy.', complete: Boolean(profile.bio.trim()), weight: 20 },
      { label: 'Contact details', detail: 'Give athletes a direct way to reach you.', complete: Boolean(profile.contactEmail.trim() || profile.phoneNumber.trim()), weight: 15 },
      { label: 'Social presence', detail: 'Add a public link for more context.', complete: Boolean(profile.instagramUrl.trim() || profile.tiktokUrl.trim() || profile.youtubeUrl.trim() || profile.xUrl.trim()), weight: 10 },
    ];
  ...
  return <main className="min-h-screen pb-20 pt-8 sm:pt-10"><div className="page-shell max-w-5xl">
    ...
    <section className="mt-6 overflow-hidden rounded-3xl border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-brand-50 p-6 shadow-sm sm:p-8" aria-labelledby="coach-readiness-title"><div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0 flex-1"><div className="flex items-center gap-2 text-emerald-700"><Sparkles className="size-4" /><p className="eyebrow text-emerald-700">Profile readiness</p></div><div className="mt-2 flex flex-wrap items-baseline gap-x-3 gap-y-1"><h2 id="coach-readiness-title" className="text-2xl font-black tracking-tight text-slate-950">{readiness.points === 100 ? 'Recruiter-ready presence' : 'Build trust with athletes'}</h2><span className="text-sm font-extrabold text-slate-500">{readiness.points}% complete</span></div><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{readiness.points === 100 ? 'Your public presence gives athletes the context and contact options they need to start a conversation.' : 'Complete your public profile so athletes can quickly understand your program and feel confident connecting.'}</p><div className="mt-4 h-2 overflow-hidden rounded-full bg-white/80" role="progressbar" aria-label="Coach profile completion" aria-valuemin={0} aria-valuemax={100} aria-valuenow={readiness.points}><div className="h-full rounded-full bg-gradient-to-r from-emerald-600 to-brand-500 transition-all" style={{ width: `${readiness.points}%` }} /></div></div><div className="w-full rounded-2xl border border-white/80 bg-white/80 p-4 lg:max-w-sm"><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-slate-500">Next best steps</p>{readiness.nextSteps.length ? <ul className="mt-3 space-y-3">{readiness.nextSteps.map((step) => <li key={step.label} className="flex gap-2.5"><Circle className="mt-0.5 size-4 shrink-0 text-slate-300" /><span><strong className="block text-sm font-bold text-slate-800">{step.label}</strong><span className="block text-xs leading-5 text-slate-500">{step.detail}</span></span></li>)}</ul> : <div className="mt-3 flex gap-2.5 text-sm font-semibold text-emerald-700"><CheckCircle2 className="size-5 shrink-0" />Everything important is covered.</div>}</div></div><div className="mt-6 grid gap-2 border-t border-emerald-100 pt-5 sm:grid-cols-2 lg:grid-cols-3">{readiness.checks.map((check) => <div key={check.label} className="flex items-center gap-2 text-xs font-semibold text-slate-600">{check.complete ? <CheckCircle2 className="size-4 text-emerald-500" /> : <Circle className="size-4 text-slate-300" />}{check.label}</div>)}</div></section>
