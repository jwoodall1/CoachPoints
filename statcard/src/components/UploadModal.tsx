...
type UploadModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSave: (image: string) => Promise<void> | void;
};
const CROP_SIZE = 280;
...
export default function UploadModal({ isOpen, onClose, onSave }: UploadModalProps) {
  ...
  const scale = naturalSize.width
    ? Math.max(CROP_SIZE / naturalSize.width, CROP_SIZE / naturalSize.height) * zoom
    : 1;
  ...
  const clampOffset = (next: { x: number; y: number }, nextZoom = zoom) => {
    ...
    const nextScale =
      Math.max(CROP_SIZE / naturalSize.width, CROP_SIZE / naturalSize.height) * nextZoom;
      ...
      <div className="w-full max-w-lg rounded-[2rem] bg-white p-6 shadow-2xl sm:p-8">
        <div className="mb-6 flex items-start justify-between">
          <div>
            ...
            <h2 className="mt-2 text-xl font-black text-slate-950">Profile photo</h2>
            <p className="mt-1 text-sm text-slate-500">
              Position and zoom your image inside the circle.
            </p>
          </div>
          <button
            type="button"
        ...
        {!source ? (
          <label className="grid cursor-pointer place-items-center rounded-3xl border-2 border-dashed border-slate-300 bg-slate-50 px-6 py-14 text-center transition hover:border-brand-400 hover:bg-brand-50">
            <span className="grid size-12 place-items-center rounded-2xl bg-white text-brand-700 shadow-sm">
              <ImagePlus className="size-5" />
            </span>
            <span className="mt-4 text-sm font-extrabold text-slate-800">Choose a photo</span>
            <span className="mt-2 text-xs text-slate-500">JPG, PNG, or WebP</span>
            <input
              className="sr-only"
              type="file"
          ...
          <>
            <div
              ...
              className="relative mx-auto size-70 touch-none cursor-grab overflow-hidden rounded-full bg-slate-200 shadow-inner ring-8 ring-slate-100 active:cursor-grabbing"
            >
              <img
                ref={imageRef}
                src={source}
            ...
            <label className="mx-auto mt-6 block max-w-70 text-sm font-bold text-slate-700">
              Zoom
              <input
                type="range"
                min="1"
            ...
            <label className="mt-4 block text-center text-sm font-extrabold text-brand-600 hover:text-brand-700">
