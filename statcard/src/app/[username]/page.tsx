...
// React cache prevents metadata and page rendering from issuing duplicate profile queries.
const getPublicProfile = cache(async (username: string) => {
  const athleteQuery = supabase
    .from('profiles')
    .select('id, first_name, last_name, account_type, height, weight, graduating_class, high_school, gpa, sport, position, bio, avatar_url, hudl_highlight_url, phone_number, contact_email, instagram_url, tiktok_url, youtube_url, x_url, stats, measurables')
    .eq('username', username)
    .maybeSingle();
  const coachQuery = supabase
      .from('coachprofiles')
      .select('id, first_name, last_name, college_university, sport, bio, avatar_url, phone_number, contact_email, instagram_url, tiktok_url, youtube_url, x_url')
      .eq('username', username)
      .maybeSingle();
  ...
  const profile = athleteProfile
    ...
    : coachProfile
      ? { ...coachProfile, account_type: 'coach' as const, height: null, weight: null, graduating_class: null, high_school: null, gpa: null, sport: coachProfile.sport, position: null, hudl_highlight_url: null, stats: null, measurables: null }
      : null;
...
/** Gives each public profile a descriptive browser title without a second database read. */
export async function generateMetadata({ params }: ProfilePageProps): Promise<Metadata> {
  const { username } = await params;
  const { profile } = await getPublicProfile(username);
  if (!profile) return { title: 'Profile not found | Athlio' };
  ...
  return {
    title: `${name} | Athlio`,
    description: `${role} profile for ${name} on Athlio.`,
  };
}

/** Loads and renders either an athlete or coach profile for a public username. */
export default async function AthleteProfile({ params }: ProfilePageProps) {
  const { username } = await params;
