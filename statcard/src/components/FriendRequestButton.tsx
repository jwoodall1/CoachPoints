...
export default function FriendRequestButton({
  ...
  if (!ready)
    return (
      <button
        type="button"
        disabled
        className="rounded-xl border border-white/15 bg-white/10 px-5 py-2.5 text-sm font-semibold text-slate-300"
      >
        Loading...
      </button>
    );
  if (!userId)
    return (
      <Link
        href="/login"
        className="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-blue-950/30 transition hover:bg-blue-500"
      >
        Sign in to follow
      </Link>
    );
  ...
  const updateConnection = async () => {
    ...
    if (!connection) {
      ({ error: mutationError } = await supabase.rpc('send_friend_request', {
        target_user_id: targetUserId,
      }));
    } else if (isIncoming) {
      ({ error: mutationError } = await supabase.rpc('accept_friend_request', {
      ...
      const result = await supabase.rpc('delete_friend_connection', {
        connection_id: connection.id,
    ...
    <div className="text-center">
      <div className="flex flex-wrap justify-center gap-2">
        <button
          ...
          className={`inline-flex min-h-11 items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-extrabold transition disabled:cursor-wait disabled:opacity-60 ${buttonClass}`}
          aria-label={actionLabel}
          title={isOutgoing || isFriend ? actionLabel : undefined}
        >
          {isFriend ? <Check className="size-4" /> : <UserPlus className="size-4" />}
          {label}
        </button>
        {isFriend && (
          <Link
            href={`/messages/${targetUserId}`}
            className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-extrabold text-white shadow-lg shadow-brand-950/30 transition hover:-translate-y-0.5 hover:bg-brand-500"
          >
      ...
      {isFriend && (
        <div className="mt-2">
          <OnlineStatus userId={targetUserId} compact />
        </div>
      )}
      {error && (
        <p role="alert" className="mt-2 max-w-56 text-xs font-medium text-rose-200">
          {error}
        </p>
      )}
    </div>
