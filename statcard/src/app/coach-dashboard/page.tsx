...
type CoachProfile = { firstName: string; lastName: string; username: string; collegeUniversity: string; sport: string; position: string; bio: string; phoneNumber: string; contactEmail: string; instagramUrl: string; tiktokUrl: string; youtubeUrl: string; xUrl: string };
const emptyProfile: CoachProfile = { firstName: '', lastName: '', username: '', collegeUniversity: '', sport: '', position: '', bio: '', phoneNumber: '', contactEmail: '', instagramUrl: '', tiktokUrl: '', youtubeUrl: '', xUrl: '' };
...
export default function CoachDashboardPage() {
  ...
  useEffect(() => {
    ...
    let active = true;
    const load = async () => {
      const { data } = await supabase.from('coachprofiles').select('first_name, last_name, username, college_university, sport, position, bio, phone_number, contact_email, instagram_url, tiktok_url, youtube_url, x_url').eq('id', user.id).maybeSingle();
      if (!active) return;
      setProfile({ firstName: data?.first_name ?? user.user_metadata.first_name ?? '', lastName: data?.last_name ?? user.user_metadata.last_name ?? '', username: data?.username ?? user.user_metadata.username ?? '', collegeUniversity: data?.college_university ?? '', sport: data?.sport ?? '', position: data?.position ?? '', bio: data?.bio ?? '', phoneNumber: data?.phone_number ?? '', contactEmail: data?.contact_email ?? user.email ?? '', instagramUrl: data?.instagram_url ?? '', tiktokUrl: data?.tiktok_url ?? '', youtubeUrl: data?.youtube_url ?? '', xUrl: data?.x_url ?? '' });
      setLoading(false);
    };
    void load();
    return () => { active = false; };
  ...
  return <main className="min-h-screen pb-20 pt-8 sm:pt-10"><div className="page-shell max-w-5xl">
    ...
    <ProfileReadinessPopup checks={readiness.checks} points={readiness.points} recruiterReady={readiness.points === 100} variant="coach" />
