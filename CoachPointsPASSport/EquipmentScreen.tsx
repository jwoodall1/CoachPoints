...
export default function EquipmentScreen({ client, onBack }: { client: SupabaseClient; onBack: () => void }) {
  ...
  useEffect(() => { alive.current = true; client.rpc('get_equipment_teams').then(({ data, error }) => { if (!alive.current) return; if (error) setError('Could not load institutions. Retry by reopening Equipment.'); else { setTeams(data ?? []); setTeam(data?.[0]?.id ?? ''); } }); return () => { alive.current = false; scanVersion.current++; uid.current = ''; void cancelEquipmentScan(); }; }, [client]);
  ...
  async function save() {
    ...
    catch { setError('Unable to save. Check your connection and institution access. If this tag was already registered, cancel and rescan it.'); }
  ...
  return <SafeAreaView style={s.safe}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={s.content}>
    ...
    {!editing ? <><Text style={s.label}>Institution / team</Text>{teams.map(t => <Pressable disabled={busy} key={t.id} onPress={() => { setTeam(t.id); setError(''); }} style={[s.chip, team === t.id && s.selected]}><Text style={team === t.id ? s.white : undefined}>{t.name}</Text></Pressable>)}{!teams.length && <Text>Approved institution membership is required. Contact your institution administrator.</Text>}
