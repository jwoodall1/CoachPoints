...
export default function FriendRequestButton({ targetUserId, targetName }: { targetUserId: string; targetName: string }) {
  ...
  return <div className="text-center">
    <button type="button" onClick={updateConnection} disabled={loading || saving || unavailable} className={`rounded-xl px-5 py-2.5 text-sm font-semibold transition disabled:cursor-wait disabled:opacity-60 ${buttonClass}`} aria-label={actionLabel} title={isOutgoing || isFriend ? actionLabel : undefined}>
