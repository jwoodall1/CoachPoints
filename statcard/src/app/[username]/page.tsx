...
const getPublicProfile = cache(async (username: string) => {
  ...
  const coachQuery = supabase
    ...
    .eq('username', username)
    .maybeSingle();
  const [{ data: athleteProfile, error: athleteError }, { data: coachProfile, error: coachError }] =
    await Promise.all([athleteQuery, coachQuery]);
  const profile = athleteProfile
    ? { ...athleteProfile, college_university: null }
    : coachProfile
      ? {
          ...coachProfile,
          account_type: 'coach' as const,
          height: null,
          weight: null,
...
export async function generateMetadata({ params }: ProfilePageProps): Promise<Metadata> {
  ...
  return {
    title: `${name} | CoachPoints`,
    description: `${role} profile for ${name} on CoachPoints.`,
  };
}
...
export default async function PublicProfile({ params }: ProfilePageProps) {
  ...
  const measurables =
    profile.measurables &&
    typeof profile.measurables === 'object' &&
    !Array.isArray(profile.measurables)
      ? Object.entries(profile.measurables)
      : [];
  const hasHudlHighlights = Boolean(
    profile.hudl_highlight_url || profile.hudl_secondary_urls?.length,
  );
  const {
    data: { publicUrl: fallbackAvatarUrl },
  } = supabase.storage.from('avatars').getPublicUrl(`${username}/profile.png`);
  ...
  const details = [
    ...
    profile.graduating_class && {
      icon: GraduationCap,
      label: `Class of ${profile.graduating_class}`,
    },
  ...
  return (
    <main className="min-h-screen pb-20">
      <section className="relative overflow-hidden bg-slate-950 text-white">
        ...
        <div
          className={`absolute -right-20 -top-32 size-[28rem] rounded-full blur-3xl ${isCoach ? 'bg-emerald-500/20' : 'bg-brand-600/25'}`}
        />
        <div className="page-shell relative py-12 sm:py-16">
          <div className="grid items-center gap-8 md:grid-cols-[auto_1fr] md:gap-10">
            <div className="mx-auto rounded-full border-4 border-white/15 bg-white/5 p-1 shadow-2xl md:mx-0">
              <ProfileAvatar src={avatarUrl} name={name} preload />
            </div>
            <div className="text-center md:text-left">
              <div
                className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.18em] ${isCoach ? 'border-emerald-300/25 bg-emerald-400/10 text-emerald-200' : 'border-brand-300/25 bg-brand-400/10 text-brand-200'}`}
              >
                {isCoach ? <UserRound className="size-3.5" /> : <Trophy className="size-3.5" />}
                {isCoach ? 'Coach profile' : 'Athlete profile'}
              </div>
              <h1 className="mt-4 text-4xl font-black tracking-[-0.04em] sm:text-5xl lg:text-6xl">
                {name}
              </h1>
              <p className="mt-2 text-sm font-semibold text-slate-400">@{username}</p>
              {details.length > 0 && (
                <div className="mt-5 flex flex-wrap justify-center gap-x-5 gap-y-2 md:justify-start">
                  {details.map(({ icon: Icon, label }) => (
                    ...
                    >
                      <Icon
                        className={`size-4 ${isCoach ? 'text-emerald-300' : 'text-brand-300'}`}
                      />
                      {label}
                    </span>
                  ))}
              ...
              <div className="mt-7 flex flex-wrap justify-center gap-3 md:justify-start">
                <FriendRequestButton targetUserId={profile.id} targetName={name} />
                {!isCoach && <AddToListButton athleteId={profile.id} prominent />}
              </div>
            </div>
          </div>
          {!isCoach && (
            <div className="mt-10 grid grid-cols-2 gap-3 border-t border-white/10 pt-7 sm:grid-cols-4">
              {profile.height && <HeroMetric icon={Ruler} label="Height" value={profile.height} />}
              {profile.weight && <HeroMetric icon={Scale} label="Weight" value={profile.weight} />}
              {profile.gpa && <HeroMetric icon={BookOpen} label="GPA" value={profile.gpa} />}
              {profile.graduating_class && (
                <HeroMetric
                  icon={GraduationCap}
                  label="Graduates"
            ...
            {!isCoach && (
              <section className="surface-card p-6 sm:p-8">
                <SectionHeading
                  icon={Gauge}
                  eyebrow="Performance"
                  title="Stats that define the game"
                />
                {stats.length ? (
                  <dl className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
                    {stats.map(([label, value]) => (
                      <DataTile key={label} label={label} value={String(value)} />
                    ))}
                  </dl>
                ) : (
                  <EmptyData text="Performance statistics will be added soon." />
                )}
              </section>
            )}
            {!isCoach && (
              <section className="surface-card p-6 sm:p-8">
                <SectionHeading icon={Ruler} eyebrow="Athletic profile" title="Measurables" />
                {measurables.length ? (
                  <dl className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
                    {measurables.map(([label, value]) => (
                      <DataTile key={label} label={label} value={String(value)} compact />
                    ))}
                  </dl>
                ) : (
                  <EmptyData text="Measurables will be added soon." />
                )}
              </section>
            )}
            {hasHudlHighlights && (
              <section className="surface-card p-6 sm:p-8">
                <SectionHeading icon={Video} eyebrow="Film" title="Hudl highlight reel" />
                <div className="mt-6">
                  <HudlHighlight
                    primaryUrl={profile.hudl_highlight_url}
                    secondaryUrls={profile.hudl_secondary_urls}
                  />
                </div>
              </section>
            )}
...
function DataTile({
  ...
  return (
    <div className="rounded-2xl border border-slate-200/80 bg-gradient-to-br from-slate-50 to-white p-5">
      <dt className="truncate text-[10px] font-extrabold uppercase tracking-[0.14em] text-slate-400">
        {label}
      </dt>
      <dd
        className={`mt-2 font-black tracking-tight text-slate-950 ${compact ? 'text-2xl' : 'text-3xl'}`}
      >
        {value}
      </dd>
    </div>
...
function EmptyData({ text }: { text: string }) {
  return (
    <div className="mt-6 rounded-2xl border border-dashed border-slate-300 bg-slate-50/70 px-5 py-9 text-center text-sm font-medium text-slate-500">
      {text}
    </div>
  );
}
